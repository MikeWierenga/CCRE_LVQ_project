...
class Test_testcre(unittest.TestCase):
    ...
    def test_unrelated_to_related_gaussian_distribution(self):
        ...
        y = np.random.normal(0,2, 100).reshape(-1,1)
        ...
        for i in np.linspace(0.01, 1, 10):
            ...
            mean_x = np.mean(new_x)
            sigma = np.std(new_x)
            sigma2_X = np.var(new_x)
           

            new_y = new_data[:, 1].reshape(-1,1)
            sigma2_Y = np.var(new_y)
            ...
            expect_value_cre_yx = integrate.dblquad(ccre_distance.calculate_expectation_value, -np.inf, np.inf, 0, np.inf, args=(mean_x, sigma, sigma2_X, cov_conditional_dist, cre_class))
