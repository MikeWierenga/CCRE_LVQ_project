...
class Main():
    ...
    def main(self, connections):
        ...
        x = np.array(df.iloc[connections[0], 2:-1]).astype(float).reshape(-1,1)
        y = np.array(df.iloc[connections[1], 2:-1]).astype(float).reshape(-1,1)
        ...
        if self.created_cre_file:
            with open(f"CCRE_distances.csv", "a", newline="") as file:
...
if __name__ == "__main__":
    ...
    pool.map(main_class.main, connections)
